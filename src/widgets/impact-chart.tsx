  ...
  async update(config: Config) {
    ...
    const chartList = indicators.map((indicator, i) => {
      ...
      const rectList = results.map((result, j) => {
        const y =
          cellOffsetY + cellHeaderHeight + j * barBoxHeight + barMarginY;
        return (
          <Bar
...
const ResponsiveSVG = ({ responsive, width, height, childrens }: SVGProps) => {
  ...
  return (
    <div
      className={responsive ? classes.parentSVGResponsive : classes.parentSVG}
    >
      <svg
...
async function selectIndicators(
  ...
  if (!model) return [];
  const _codes =
    !c || !c.indicators || c.indicators.length === 0
...
async function getSectorResults(
  model: WebModel,
  c: Config,
): Promise<SectorResult[]> {
  if (!c || !c.sectors) return [];
...
async function getNormalizationTotals(
  model: WebModel,
  c: Config,
): Promise<number[]> {
...
function makeRelative(results: SectorResult[], indicators: Indicator[]) {
  ...
  indicators.forEach((indicator) => {
    ...
    const maxval = results.reduce(
      (max, r) => Math.max(max, Math.abs(r.profile[i])),
      0,
    );
    for (const r of results) {
      r.profile[indicator.index] = maxval === 0 ? 0 : r.profile[i] / maxval;
    }
  });
}
