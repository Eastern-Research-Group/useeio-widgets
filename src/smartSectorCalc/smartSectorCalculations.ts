...
export function sortedSectorCodeList(
...
export function sortedSeriesList(
  ...
  sortTopTen.forEach((s) => {
    s._smartSectors.forEach((l) => {
      ...
      const value =
        impactSelector === "impact_per_purchase"
  ...
  return uniqueSortedMappingGroupNoDuplicates.reduce(
    (acc, groupName) => {
...
export function SumSmartSectorTotal(
  ...
  const sumSmartSectorTotalParts: SumSmartSectorTotalParts[] = [];
  ...
  smartSectorList.forEach((item) => {
    ...
    } else groupToSmartSectors.get(group).push(item);
  });
  ...
  if (items?.length > 0) {
    ...
    items.forEach((v) => {
      ...
      const sumSmartSectorTotalPart = new SumSmartSectorTotalParts(
  ...
  uniqueGroupIds.forEach((g) => {
    if (groupIndicator !== g) {
      sumSmartSectorTotalParts.forEach((s) => {
        ...
        const smartSector = items.find(
          (v) => v._smartSector.sumSectorCode === s._sectorCode,
        );
        ...
        if (smartSector) {
          ...
          const newSmartSector = new SmartSector({
            ...
            sumImpactPerDollar:
              impactSelector === "impact_per_purchase" ? 0 : undefined,
            sumtotalImpact:
              impactSelector !== "impact_per_purchase" ? 0 : undefined,
            sumPurchasedGroup: g,
          });
...
export function selectSectorName(
  sectorId: string,
  sectorList: Sector[],
): string {
...
export function uniqueSortedMappingGroupNoDuplicatesList(
  ...
  if (groupSelection === "group_summary") {
    sortedSectorMappingByGroup = sectorMappingList.sort(
      (a: SectorMapping, b: SectorMapping): any => {
    ...
    groupMapping = sortedMappingGroupList.filter(
      (value, index) => sortedMappingGroupList.indexOf(value) === index,
    );
  ...
  const orderMap: { [key: string]: number } = customOrder.reduce(
    (map, name, index) => {
      map[name] = index;
      return map;
    },
