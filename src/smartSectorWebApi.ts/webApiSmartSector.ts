      ...
      req.onload = () => {
        if (req.status === 200) {
          try {
            ...
            reject("failed to parse response for: " + url + ": " + err);
          }
        } else {
...
export class WebModelSmartSector {
   ...
   */
  async sectorContributionToImpactGhg(
    fileName: string,
  ): Promise<SectorContributionToImpact[]> {
    return await this.api.getJson(
      this.modelId,
      "sector_contribution_to_impact/" + fileName,
    );
  }
   ...
   * Returns the sector_contribution_to_impact_ghg of the smart sector EEIO model.
   */
  async sectorContributionToImpactGhgAPI(
    fileName: string,
  ): Promise<SectorContributionToImpact[]> {
    ...
    if (snes != null || snes != undefined) {
      ...
      if (fieldValueData != undefined) {
        ...
        const data: SectorContributionToImpact[] = await this.api.api(
          this.modelId,
          "sector_contribution_to_impact/" + fileName,
        );
   ...
   */
  async sectorContributionToImpactRankedGhgAPI(
    fileName: string,
  ): Promise<SectorContributionToImpact[]> {
    ...
    if (snes != null || snes != undefined) {
      ...
      if (fieldValueData != undefined) {
        ...
        const data: SectorContributionToImpact[] = await this.api.api(
          this.modelId,
          "sector_contribution_to_impact_ranked/" + fileName,
        );
        await snes.set(fileName, JSON.stringify(data));
        return data;
      }
  ...
  async percentContribution(fileName: string): Promise<PercentContribution[]> {
    ...
    if (snes != null || snes != undefined) {
      ...
      if (fieldValueData != undefined) {
        ...
        const data: PercentContribution[] = await this.api.api(
          this.modelId,
          "percent_contribution_to_impact_by_sector/" + fileName,
        );
        await snes.set(fileName, JSON.stringify(data));
        return data;
      }
  ...
  async sectorMapping(): Promise<SectorMapping[]> {
    if (!this._sectoMapping)
      this._sectoMapping = await this.api.getJson(
        this.modelId,
        "sector_mapping",
      );
  ...
  async sectorRecordList(): Promise<DataRow[]> {
    if (!this._sectorListTable)
      this._sectorListTable = await this.api.getJson(
        this.modelId,
        "sectorInfo",
      );
  ...
  async impactOutPut(): Promise<ImpactOutput[]> {
    if (!this._sectorOutput)
      this._sectorOutput = await this.api.getJson(
        this.modelId,
        "commodity_output",
      );
